...
class ExcelGenerator:
    ...
    def _summary(cls, wb: Workbook, m: Dict[str, Any]) -> None:
        ...
        if health.get("deductions"):
            for d in health["deductions"]:
                findings.append((d.get("category", "Governance"), f"[{d.get('impact')}] {d.get('title')}: {d.get('description')}"))
        if not findings:
            findings.append(("Status", "No major findings detected by deterministic checks."))
            findings.append(("Status", "No major risk factors detected by deterministic checks."))

        start = 4 + len(rows) + 2
        ws.cell(start, 1, "Important Findings").fill = SECTION_FILL
        ws.cell(start, 1, "Model Health & Governance Deductions").fill = SECTION_FILL
    ...
    def _model_inventory(cls, wb: Workbook, m: Dict[str, Any]) -> None:
        ...
        child_used: Dict[str, set] = {}
        for u in m.get("object_usage", []):
            if u.get("Used") == "Yes":
                tname = u.get("Table")
                child_used.setdefault(tname, set()).add(u.get("Usage Type", "Used"))
        ...
        for t in m.get("tables", []):
            ...
            c_types = child_used.get(name, set())
            if "Direct + Indirect" in c_types or ("Direct" in c_types and "Indirect" in c_types):
                u_type = "Direct + Indirect (via child objects)"
                used_str = "Yes"
            elif "Direct" in c_types:
                u_type = "Direct (via child objects)"
                used_str = "Yes"
            elif "Indirect" in c_types:
                u_type = "Indirect (via child objects)"
                used_str = "Yes"
            else:
                u_type = "Unused"
                used_str = "No"
            ...
            rows.append({
                "Table": name, "Object": name, "Object Type": "Table",
                "Data Type": "", "DAX Expression": "", "Hidden": t.get("Is Hidden", "No"),
                "Used": used_str, "Usage Type": u_type, "Pages": "", "Visuals": ""
            })
        ...
        for c in m.get("columns", []):
            ...
            rows.append({
                "Table": table, "Object": obj, "Object Type": typ,
                "Data Type": c.get("Data Type", ""),
                "DAX Expression": calc_map.get((table, obj), {}).get("DAX Expression", "") if is_calc else "",
                "Hidden": c.get("Is Hidden", "No"), "Used": u.get("Used", "No"),
                "Usage Type": u.get("Usage Type", "Unused"),
                "Pages": "; ".join(sorted(loc["pages"])), "Visuals": "; ".join(sorted(loc["visuals"]))
            })
        ...
        for x in m.get("measures", []):
            ...
            rows.append({
                "Table": table, "Object": obj, "Object Type": "Measure",
                "Data Type": "", "DAX Expression": x.get("DAX Expression", ""),
                "Hidden": x.get("Is Hidden", "No"), "Used": u.get("Used", "No"),
                "Usage Type": u.get("Usage Type", "Unused"),
                "Pages": "; ".join(sorted(loc["pages"])), "Visuals": "; ".join(sorted(loc["visuals"]))
            })
    ...
    def _relationships(cls, wb: Workbook, m: Dict[str, Any]) -> None:
        ...
        for r in m.get("relationships", []):
            ...
            rows.append({
                "Type": "Relationship",
                "From": f"{r.get('From Table','')}[{r.get('From Column','')}]",
                "To": f"{r.get('To Table','')}[{r.get('To Column','')}]",
                "Cardinality": r.get("Cardinality", ""),
                "Filter Direction": r.get("Filter Direction", ""),
                "Status": r.get("Status", "")
            })
        for d in m.get("dax_dependencies", []):
            ...
            rows.append({
                "Type": "DAX Dependency",
                "From": f"{d.get('Source Table','')}[{d.get('Source Object','')}]",
                "To": f"{d.get('Referenced Table','')}[{d.get('Referenced Object','')}]",
                "Cardinality": "", "Filter Direction": "",
                "Status": d.get("Resolution Status", "Resolved")
            })
    ...
    def _sources(cls, wb: Workbook, m: Dict[str, Any]) -> None:
        ...
        for s in m.get("data_sources", []):
            ...
            rows.append({
                "Table": s.get("Table", ""),
                "Source Type": s.get("Source Type", "Other"),
                "Source Details": details,
                "Power Query / M Expression": s.get("Power Query / M Expression", "")
            })
    ...
    def _dax(cls, wb: Workbook, m: Dict[str, Any]) -> None:
        ...
        for x in analysis:
            ...
            rows.append({
                "Table": x.get("Table", ""),
                "Object": x.get("Object Name", ""),
                "Object Type": x.get("Object Type", ""),
                "Original DAX": x.get("Original DAX", x.get("DAX Expression", "")),
                "Complexity Score": x.get("Complexity Score", 0),
                "Static Issues": imp.get("Issue", x.get("Issue", "None")) if imp.get("Source") == "Static Rule" else "None",
                "AI Issues": imp.get("Issue", "") if imp.get("Source") == "AI" else "None",
                "Suggested DAX": imp.get("Suggested DAX", "No safe automatic rewrite"),
                "Suggestion Source": imp.get("Source", "Static Rule"),
                "AI Confidence": imp.get("Confidence", "N/A"),
                "Validation Status": imp.get("Validation Status", "Validated"),
                "Requires Manual Review": "Yes" if imp.get("Requires Manual Validation") or imp.get("Validation Status") in ("Needs Review", "Invalid") else "No",
                "Dependency Count": x.get("Dependency Count", 0),
                "Unresolved References": "Yes" if x.get("Analysis Status") == "Unresolved" else "No",
                "Circular Dependency": "No",
                "Usage": x.get("Usage Type", "Unused"),
                "Direct Visual Count": x.get("Direct Usage Count", 0),
                "Indirect Usage Count": x.get("Indirect Usage Count", 0),
            })
        ...
        if not rows:
            for x in improvements:
                rows.append({
                    ...
                    "Object": x.get("Object Name", ""),
                    "Object Type": x.get("Object Type", ""),
                    "Original DAX": x.get("Original DAX", ""),
                    "Complexity Score": 0,
                    "Static Issues": x.get("Issue", ""),
                    "AI Issues": x.get("Issue", "") if x.get("Source") == "AI" else "None",
                    "Suggested DAX": x.get("Suggested DAX", ""),
                    "Suggestion Source": x.get("Source", "Static Rule"),
                    "AI Confidence": x.get("Confidence", "N/A"),
                    "Validation Status": x.get("Validation Status", "Validated"),
                    "Requires Manual Review": "Yes" if x.get("Requires Manual Validation") else "No",
                    "Dependency Count": 0,
                    "Unresolved References": "No",
                    "Circular Dependency": "No",
                    "Usage": "Unused",
                    "Direct Visual Count": 0,
                    "Indirect Usage Count": 0,
                })

        headers = [
            "Table", "Object", "Object Type", "Original DAX", "Complexity Score",
            "Static Issues", "AI Issues", "Suggested DAX", "Suggestion Source",
            "AI Confidence", "Validation Status", "Requires Manual Review",
            "Dependency Count", "Unresolved References", "Circular Dependency",
            "Usage", "Direct Visual Count", "Indirect Usage Count"
        ]
        cls._table(wb, "DAX Analysis", "DAX Quality, Complexity, Recommendations & AI Optimization", rows, headers)
